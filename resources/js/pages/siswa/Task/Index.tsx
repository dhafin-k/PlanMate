...
import { Plus, Pencil, Trash2, CheckCircle2, XCircle, List, Calendar, Search, ChevronLeft, ChevronRight, CheckCircle, Ghost, Trash, AlertCircle, ClipboardCheck, CircleCheck, CircleDashed, ListCheck, PlayCircle, ClipboardList, Eye } from 'lucide-react';
...
export default function TasksIndex({ tasks, lists, filters, flash }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (editingTask) {
            put(`/siswa/tasks/${editingTask.id}`, {
                onSuccess: () => {
                    setIsOpen(false);
                    reset();
                    setEditingTask(null);
                            ...
                            <DialogContent className='sm:max-w-[425px]'>
                                <DialogHeader>
                                    <DialogTitle className='text-xl'>
                                        {editingTask ? 'Review Tugas' : 'Tambah Tugas Baru'}
                                    </DialogTitle>
